...
char Check(string &str, int n, int id) {
    if (str[id] == '1') return '1';
    if (n == 1) return str[id];
    if (id == 0) {
        if (str[id + 1] == '1') return '1';
    } else if (id == n - 1) {
        if (str[id - 1] == '1') return '1';
    } else {
        if (str[id - 1] - '0' + str[id + 1] - '0' == 1) return '1';
    }
    return '0';
}
int main() {
    ...
    Test {
        ...
        v.push_back(s);
        ll ok = 1;
        for (int i = 1; true; i++ , ok++) {
            string tmp = "";
            for (int j = 0; j < n; j++) {
                tmp += Check(v[i - 1] , n , j);
            }
            v.push_back(tmp);
            if (v[i] == v[i - 1] || ok > m) break;
        }
        if (ok < m) {
            cout << v.back() << "\n";
        } else {
